...
Codex 客户端**不会从代理拿 context window**，它有自己一份模型元数据表，未列名的统一兜底成 ~256K——所以即便代理转发到 `mimo-v2.5-pro[1m]` 或 `deepseek-v4-pro`，左下角显示也只有 258K。要让 Codex 真的按 1M 算，必须在 `config.toml` 里**显式写 `model_context_window`**。

最快方式：

```bash
mimo2codex --long-context print-config              # MiMo 1M (mimo-v2.5-pro[1m])
mimo2codex --model ds --long-context print-config   # DeepSeek 1M (默认即 1M)
mimo2codex --long-context print-cc-switch           # cc-switch 粘贴片段
```

输出片段已经把 `model_context_window = 1000000`（DeepSeek 还附 `model_max_output_tokens = 393216`）写好了，连同其他可选模型的备注一起放在 toml 里——cc-switch 用户在文本框里把 `model = "..."` 那行换成想要的就行，**不用改代理**。

写入 `~/.codex/config.toml` 后**完全退出 + 重启 Codex**（桌面端走系统托盘退出，不只是关窗口），左下角 context 就从 258K 变成 1M。
