...
- **[new]** **Global web search forwarding switch**：added a toggle under Codex → Thinking & Override to disable MiMo web_search forwarding for all requests. Also available via `--no-web-search` / `MIMO2CODEX_DISABLE_WEB_SEARCH=1`. Useful when automatic web search is billed more than expected.
